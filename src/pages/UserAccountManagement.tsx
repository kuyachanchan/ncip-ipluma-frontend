...
const SQL_INJECTION_PATTERN = /('|"|;|--|\/\*|\*\/|xp_|UNION|SELECT|INSERT|UPDATE|DELETE|DROP|ALTER|EXEC|EXECUTE|CAST|CONVERT|CHAR|NCHAR|VARCHAR|NVARCHAR|ALTER|BEGIN|END|DECLARE|FETCH|KILL|OPEN|SYSOBJECTS|SYSCOLUMNS)/i;
...
const UserAccountManagement: React.FC = () => {
  ...
  const [sendingEmailIds, ] = useState<Set<string>>(new Set());
  ...
  const handleCreateSubmit = async (e: React.FormEvent) => {
    ...
    if (hasSQLInjection(password)) {
      setPasswordError('Invalid characters detected in password.');
      return;
    }
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    if (password && hasSQLInjection(password)) {
      setPasswordError('Invalid characters detected in password.');
      return;
    }
    ...
    try {
      const roles: Array<Role> = role ? [role] : [];
  ...
  return (
    <div className="relative min-h-screen bg-[#E7F2EF] p-8">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium text-gray-900">{selectedUsers.size} selected</span>
                </div>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center justify-end gap-1">
                            ...
                            <button
                              ...
                              className="p-2 text-gray-500 hover:text-[#708993] hover:bg-[#E7F2EF] rounded-lg transition-colors"
                              title="Edit user"
                            >
                ...
                <div className="relative">
                  <Form.Control asChild>
                    <input
                      ...
                      required // remove this line in Edit dialog
      ...
      {/* Edit User Dialog */}
      <Dialog.Root open={editDialogOpen} onOpenChange={(open) => { setEditDialogOpen(open); if (!open) resetForm(); }}>
        <Dialog.Portal>
          <Dialog.Overlay className="fixed inset-0 bg-black/50 backdrop-blur-sm" />
          <Dialog.Content className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 border-2 border-[#A1C2BD]">
            <Dialog.Title className="flex items-center gap-3 text-2xl font-bold text-[#19183B] mb-4">
        ...
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 border-2 border-red-200">
            ...
            <div className="flex gap-3">
              <button
                onClick={() => { setDeleteDialogOpen(false); setSelectedUser(null); }}
                className="flex-1 px-6 py-3 border-2 border-[#A1C2BD] text-[#19183B] rounded-xl font-semibold hover:bg-[#E7F2EF] transition-colors"
              >
                Cancel
              </button>
